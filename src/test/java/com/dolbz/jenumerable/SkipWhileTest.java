...
public class SkipWhileTest extends JEnumerableTestBase {
	...
	public void nullPredicateNoIndex() {
		...
		JEnumerable.range(0, 5).skipWhile((Predicate<Integer>) null);
	}

	@Test
	public void nullPredicateWithIndex() {
		exception.expect(IllegalArgumentException.class);
		JEnumerable.range(0, 5).skipWhile((IndexPredicate<Integer>) null);
	}
	...
	public void predicateFailingFirstElement() {
		...
		JEnumerable<String> result = source.skipWhile(new Predicate<String>() {
			@Override
			public boolean check(final String source) {
	...
	public void predicateWithIndexFailingFirstElement() {
		...
		JEnumerable<String> result = source
				.skipWhile(new IndexPredicate<String>() {
					@Override
					public boolean check(final String source,
							final Integer index) {
